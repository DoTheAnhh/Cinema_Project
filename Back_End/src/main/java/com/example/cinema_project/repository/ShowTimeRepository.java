...
public interface ShowTimeRepository extends JpaRepository<ShowTime, Long> {
    ...
    @Query("SELECT COUNT(st) > 0 FROM ShowTime st WHERE st.cinemaRoom.id = :cinemaRoomId AND st.showDate = :showDate AND (st.showTime < :showTimeEnd AND st.showTimeEnd > :showTime)")
    boolean existsConflict(@Param("cinemaRoomId") Long cinemaRoomId, @Param("showDate") Date showDate, @Param("showTime") String showTime, @Param("showTimeEnd") String showTimeEnd);

    @Query("SELECT COUNT(st) > 0 FROM ShowTime st WHERE st.cinemaRoom.id = :cinemaRoomId AND st.showDate = :showDate AND st.id != :showTimeId AND (st.showTime < :showTimeEnd AND st.showTimeEnd > :showTime)")
